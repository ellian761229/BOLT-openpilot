...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      # self.pedal_gas_max = interp(CS.out.vEgo, [0.0, 2, 5, 10, 30, 50, 70], [0.2390, 0.2400, 0.2350, 0.2500, 0.2650, 0.2800, 0.2900])
      ...
      zero = 0.15625  # 40/256
      accGain = interp(abs(actuators.accel), [0, 1.5],[0.2, 1-zero])

      self.comma_pedal = clip(zero + accGain * actuators.accel, 0., 1)      
        ...
        if self.stoppingStateTimeWindowsActive :
          if not self.stoppingStateTimeWindowsClosing:
            ...
            if self.stoppingStateTimeWindowsActiveCounter > 0 :
              actuators.pedalStartingAdder = interp(CS.out.vEgo, [0.0, 2.0 * CV.KPH_TO_MS ,12.5 * CV.KPH_TO_MS , 25.0 * CV.KPH_TO_MS], [0.1300,0.1400, 0.1650, 0.025])
              if d > 0: 
