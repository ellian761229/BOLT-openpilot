...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      ConstAccel = interp(CS.out.vEgo, [0.5 * CV.KPH_TO_MS, 1.3 * CV.KPH_TO_MS, 3.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 18.0 * CV.KPH_TO_MS, 100.0 * CV.KPH_TO_MS], [0.025, 0.042, 0.053, 0.075, 0.12, 0.16, 0.2125])  #0.15, 0.2125
      ...
      self.comma_pedal_original = clip(interp(actuators.accel, [-0.870, 0.00, 0.30], [0.0, ConstAccel, ConstAccel+0.0250]) + accelFomula, 0., 1.)
        ...
        if self.stoppingStateTimeWindowsActive :
          if not self.stoppingStateTimeWindowsClosing:
            ...
            if self.stoppingStateTimeWindowsActiveCounter > 0 :
              actuators.pedalStartingAdder = interp(CS.out.vEgo, [0.0, 2.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 13.0 * CV.KPH_TO_MS, 25.0 * CV.KPH_TO_MS, 35.0 * CV.KPH_TO_MS], [0.011, 0.0125, 0.024, 0.098, 0.13, 0.13, 0.022]) #0.0050, 0.0200
      ...
      #braking logic
      if actuators.accel < interp(CS.out.vEgo,[2.0* CV.KPH_TO_MS, 6.0* CV.KPH_TO_MS, 10.0* CV.KPH_TO_MS, 18.0* CV.KPH_TO_MS, 30.0* CV.KPH_TO_MS, 60.0* CV.KPH_TO_MS, 80.0* CV.KPH_TO_MS, 100.0* CV.KPH_TO_MS],[-2.1, -2.0, -1.75, -1.75, -1.75, -2.0, -2.2, -2.2]) : #-0.16, -0.6
        ...
        can_sends.append(gmcan.create_regen_paddle_command(self.packer_pt, CanBus.POWERTRAIN))
        actuators.regenPaddle = True #for icon
        minMultipiler = interp(CS.out.vEgo, [20 * CV.KPH_TO_MS ,  30 * CV.KPH_TO_MS , 60 * CV.KPH_TO_MS ,120 * CV.KPH_TO_MS ], [0.851, 0.752, 0.625, 0.150])
