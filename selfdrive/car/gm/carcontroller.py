...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      self.pedal_gas_max = interp(CS.out.vEgo, [0.0, 5, 10, 30], [0.2025, 0.2325, 0.2975, 0.3650])
        ...
        if self.stoppingStateTimeWindowsActive :
          if not self.stoppingStateTimeWindowsClosing:
            ...
            if self.stoppingStateTimeWindowsActiveCounter > 0 :
              actuators.pedalStartingAdder = interp(CS.out.vEgo, [0.0, 2.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 13.0 * CV.KPH_TO_MS, 25.0 * CV.KPH_TO_MS, 35.0 * CV.KPH_TO_MS], [0.0, 0.0070, 0.0900, 0.1100, 0.1250, 0.1550, 0.250])
              if d > 0: 
