...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      self.pedal_gas_max = interp(CS.out.vEgo, [0.0, 2, 5, 10, 30, 50, 70], [0.2250, 0.2320, 0.2350, 0.2500, 0.2700, 0.2800, 0.2900])
        ...
        if self.stoppingStateTimeWindowsActive :
          if not self.stoppingStateTimeWindowsClosing:
            ...
            if self.stoppingStateTimeWindowsActiveCounter > 0 :
              actuators.pedalStartingAdder = interp(CS.out.vEgo, [0.0, 2.0 * CV.KPH_TO_MS ,12.5 * CV.KPH_TO_MS , 25.0 * CV.KPH_TO_MS], [0.1350,0.1475, 0.1650, 0.025])
              if d > 0: 
