...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      ConstAccel = interp(CS.out.vEgo, [0.5 * CV.KPH_TO_MS, 1.3 * CV.KPH_TO_MS, 3.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 18.0 * CV.KPH_TO_MS, 100.0 * CV.KPH_TO_MS], [0.024, 0.040, 0.052, 0.073, 0.118, 0.158, 0.2105])  #0.15, 0.2125
